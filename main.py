...
workout = pd.read_csv("workout_df1.csv")
diets = pd.read_csv("diets.csv")
symptoms_data = pd.read_csv('symtoms_df.csv')  # Replaced file name here

# Extract unique symptoms and sort them alphabetically
symptoms_options = symptoms_data[['Symptom_1', 'Symptom_2', 'Symptom_3']].values.flatten()
symptoms_options = [symptom.strip() for symptom in symptoms_options]  # Remove leading and trailing whitespaces
symptoms_options = list(set(symptoms_options))  # Remove duplicates
symptoms_options.sort()  # Sort the list alphabetically
...
# Helper function to handle user input symptoms
def preprocess_symptoms(symptoms):
    return [symptom.strip().lower() for symptom in symptoms if symptom.strip()]

# Helper function to retrieve description, precautions, diet, and workout for a given disease
def helper(disease):
    ...
    die = diets[diets['Disease'] == disease]['Diet'].values[:4]  # Retrieve first 4 diets
    wrkout = workout[workout['disease'] == disease]['workout'].values[:4]  # Retrieve first 4 workouts
...
def get_predicted_value(patient_symptoms):
    ...
    for item in patient_symptoms:
        symptom = item.strip().lower()
        if symptom in symptoms_options:
            input_vector[symptoms_dict[symptom]] = 5
...
@app.route("/")
def index():
    return render_template('index.html', symptoms_options=symptoms_options)

@app.route('/predict', methods=['POST'])
def predict():
    if request.method == 'POST':
        symptoms = preprocess_symptoms([request.form.get('symptom_1'), request.form.get('symptom_2'), request.form.get('symptom_3')])
        if not symptoms:
        ...
        return render_template('index.html', predicted_disease=predicted_disease, dis_des=desc,
                               my_precautions=pre, my_diet=die, workout=wrkout)
